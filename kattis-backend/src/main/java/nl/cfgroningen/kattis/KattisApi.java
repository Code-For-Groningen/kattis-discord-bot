    ...
    public CompletableFuture<UniversityScoreInformation> getUniversityScoreInformation(String universityUrl) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                statsList.getElementsByClass("divider_list-item").stream()
                        .filter(e -> !e.getElementsByClass("divider_list-item").isEmpty())
                        .filter(e -> e.getElementsByTag("span").size() == 2)
                        .forEach(e -> {
                            stats.put(e.getElementsByTag("span").get(0).text(), e.getElementsByTag("span").get(1).text());
                        });
